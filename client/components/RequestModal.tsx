...
export function RequestModal({ isVisible, onClose, isLoading, rpcResponse, rpcError }: Props) {
  return (
    <Modal isVisible={isVisible} onBackdropPress={onClose}>
      ...
      <View style={styles.innerContainer}>
        ...
        {rpcResponse && (
          <>
            <Text style={[styles.title, styles.successText]}>Request Response</Text>
            {Object.keys(rpcResponse).map((key) => (
              <Text key={key} style={styles.subtitle}>
                {key}: <Text style={styles.responseText}>{rpcResponse[key]?.toString()}</Text>
              </Text>
            ))}
          </>
        )}
        {rpcError && (
          <>
            <Text style={[styles.title, styles.failureText]}>Request Failure</Text>
